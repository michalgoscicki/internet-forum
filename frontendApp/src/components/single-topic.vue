...
<template>
  <div class="bg-blue-400 px-2rem py-1rem rounded-2xl mt-1rem shadow-md mx-1/10">
    <div class="flex">
      ...
      <Icon @click="changeVisible" name="mdi:application-edit" v-if="(props.author.id === auth.user.id || auth.user.roles.some(role => role.id === 2))  && route.name === 'topics-id'" class="text-black text-2xl mx-2"/>
      <Icon @click="deleteTopic" name="mdi:delete-forever" v-if="(props.author.id === auth.user.id || auth.user.roles.some(role => role.id === 2)) && route.name === 'topics-id'" class="text-black text-2xl"/>
      <Icon @click="followTopic" name="mdi:bookmark-plus" v-if="route.name === 'topics-id'" class="text-black text-2xl mx-2"/>
    </div>
